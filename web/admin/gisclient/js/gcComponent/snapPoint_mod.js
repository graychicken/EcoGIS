	...
	$.widget("gcComponent.snapPoint", $.ui.gcComponent, {
		...
		_create: function() {
			...
			if(typeof(gisclient.componentObjects.snapPoint) == 'object') {
				...
				var html = '<div id="editing_snap" class="noflow"><label>'+OpenLayers.i18n('Snap layer:')+'</label> <select id="snap_select_layer"><option value="0">'+OpenLayers.i18n('Snap Off')+'</option>';
				...
				html += '</select><div style="clear: both; padding: 10px 0 10px 0;"><label>'+OpenLayers.i18n('Highlight vertices')+'</label><input type="checkbox" name="highlight_vertices" value="yes"></div><div id="tollerance_container">'+OpenLayers.i18n('Tolerance')+': <span id="snap_tolerance_value">10</span><div id="snap_tolerance_selector" style="width:300px;margin-left:20px;"></div></div></div>';
		...
		highlightVertexes: function() {
			...
			if(!$('#editing_snap input[name="highlight_vertices"]').attr('checked')) return;
