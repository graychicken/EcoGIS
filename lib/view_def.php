...
class R3EcoGisCustomerViewDef {
    ...
    static private $viewDef = array(
        'building' => array(
            'sql' => "SELECT bu_id, bu_code, bu_name_1, bu_name_2, bt_id, fr_id, st_id, bu_nr_civic, bu_nr_civic_crossed, bu_build_year,
                             ...
                             mu_id, setsrid(the_geom, <SRID>) AS the_geom, ez_id, cm_id, cm_number, ecl_id
        ...
        'street_lighting' => array(
            'sql' => "SELECT sl_id, mu_id, st_id, st_name_1, st_name_2, sl_descr_1, sl_descr_2, sl_full_name_1, sl_full_name_2, setsrid(sl.the_geom, <SRID>) AS the_geom
        ...
        'global_subcategory' => array('sql' => "SELECT gs_id, ge_id, mu_id, ge_year, ge_name_1, ge_name_2, 
                                                ...
                                                gs_name_1, gs_name_2, 
                                        setsrid(gs.the_geom, <SRID>) AS the_geom
                                      FROM <ECOGIS-SCHEMA>.global_subcategory_data gs
                                      WHERE has_geometry IS TRUE AND do_id=<DOMAIN-ID>",
            'desc' => "PAES standard table (Autogenerated)",
            'geo' => array('col' => 'the_geom', 'dim' => 2, 'type' => 'MULTIPOLYGON')),
        'fraction' => array('sql' => "SELECT fr_id, fr.mu_id, mu_istat, fr_code, fr_name_1, fr_name_2, CASE fr_visible WHEN TRUE THEN 'T' WHEN FALSE THEN 'F' END AS fr_visible, setsrid(fr.the_geom, <SRID>) AS the_geom
        ...
        'street' => array('sql' => "SELECT st_id, st.mu_id, mu_istat, st_code, st_name_1, st_name_2, st_lkp_name_1, st_lkp_name_2, CASE st_visible WHEN TRUE THEN 'T' WHEN FALSE THEN 'F' END AS st_visible, setsrid(st.the_geom, <SRID>) AS the_geom
        ...
        'consumption_year_stats' => array('sql' => "SELECT co_year, et_code, co_value_kwh, co_value_spec_kwh, co_value_tep, co_value_spec_tep, 
                                                       ...
                                                       setsrid(the_geom, <SRID>) AS the_geom
    ...
    static private $mmViewDef = array('toponimy' => array('sql' => "SELECT 1 AS to_priority, mu_id AS to_id, mu_istat AS to_code, 'Comune di ' || mu_name_1 AS to_name_1, 'Gemeinde ' || mu_name_2 AS to_name_2, st_boundary(st_transform(the_geom, <SRID>)) AS the_geom
                                      ...
                                      UNION
                                      SELECT 2, fr_id, fr_code, fr_name_1 || ' - ' || mu_name_1, fr_name_2 || ' - ' || mu_name_2, st_multi(setsrid(fr.the_geom, <SRID>))
                                      FROM <COMMON-SCHEMA>.fraction fr
                                      INNER JOIN <ECOGIS-SCHEMA>.municipality mu ON fr.mu_id=mu.mu_id
                                      WHERE fr.the_geom IS NOT NULL AND mu.do_id=<DOMAIN-ID>
                                      UNION
                                      SELECT 3, st_id, st_code, COALESCE(st_lkp_name_1, st_name_1) || ' - ' || mu_name_1, COALESCE(st_lkp_name_2, st_name_2) || ' - ' || mu_name_2, st_multi(setsrid(st.the_geom, <SRID>))
                                      FROM <COMMON-SCHEMA>.street st
                                      INNER JOIN <ECOGIS-SCHEMA>.municipality mu ON st.mu_id=mu.mu_id
    ...
    static private $smViewDef = array('toponimy' => array('sql' => "
                                      SELECT 2 AS to_priority, fr_id AS to_id, fr_code AS to_code, fr_name_1 AS to_name_1, fr_name_2 AS to_name_2, st_multi(setsrid(fr.the_geom, <SRID>)) AS the_geom
                                      FROM <COMMON-SCHEMA>.fraction fr
                                      INNER JOIN <ECOGIS-SCHEMA>.municipality mu ON fr.mu_id=mu.mu_id
                                      WHERE fr.the_geom IS NOT NULL AND mu.do_id=<DOMAIN-ID>
                                      UNION
                                      SELECT 3, st_id, st_code, COALESCE(st_lkp_name_1, st_name_1), COALESCE(st_lkp_name_2, st_name_2), st_multi(setsrid(st.the_geom, <SRID>))
                                      FROM <COMMON-SCHEMA>.street st
                                      INNER JOIN <ECOGIS-SCHEMA>.municipality mu ON st.mu_id=mu.mu_id
                                      WHERE st.the_geom IS NOT NULL AND mu.do_id=<DOMAIN-ID>
                                      ",
