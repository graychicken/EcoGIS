...
<script type="text/javascript" language="javascript">
    ...
    function askDel(cod) {ldelim}
        ...
        if (confirm('{if !isset($txt.confirm)}{t}Sei sicuro, di voler cancellare questo dominio?{/t}{else}{$txt.confirm}{/if}'))
            submitForm(cod);
    {rdelim}
    ...
    function submitForm(cod) {ldelim}
        ...
        xajax_submitForm(new Array('act|del', 'name|' + cod), "formCheckDone", "formCheckError");
    {rdelim}
...
<input type="hidden" name="pg" value="1">
  <div class="function_list">
    <input type="button" name="btnAdd" value="{if !isset($btn.new)}{t}Nuovo{/t}{else}{$btn.new}{/if}" onClick="location.href='domains_edit.php?act=add';" style="height:25px;width:70px;" />
  </div>
</form>
 
<h3>{if !isset($txt.domain_title)}{t}Gestione domini{/t}{else}{$txt.domain_title}{/if} - {if !isset($txt.Totale)}{t}Totale{/t}{else}{$txt.Totale}{/if}: {$tot}</h3>
  
<form name="simpleTable" method="post" action="domains_list.php">
{$table_html}
{$navigationBar_html}
</form>
